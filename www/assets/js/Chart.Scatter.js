(function (root, factory) {
	...
	var helpers = chartjs.helpers,
		hlp = {
			...
			formatDateValue: function (date, tFormat, dFormat, useUtc) {
				...
				var hasTime = useUtc ? date.getUTCHours() + date.getUTCMinutes() + date.getUTCSeconds()	: date.getHours() + date.getMinutes() + date.getSeconds();
	...
	chartjs.ScatterNumberScale = chartjs.Element.extend({
		...
		initialize: function () {
			// this.dataRange - минимальные и максимальные значения данных | minimum and maximum data values
			// инициализируем настройки | initialize the settings
			// рассчитываем вспомогательные параметры | We expect support options
		...
		fit: function () {
			...
			this.xPadding = this.display && this.showLabels	? yLabelMaxWidth + this.padding * 2	: this.padding;
			...
			this.xPaddingRight = this.display && this.showLabels && !this.xLabelRotation ? xLabelMaxWidth / 2 : this.padding;

			this.yPadding = this.display && this.showLabels	? (this.xLabelRotation ? xLabelMaxWidth : xLabelHeight) + this.padding * 2 : this.padding;
		},
	...
	chartjs.Type.extend({
		...
		_initScale: function () {
			...
			return this.options.scaleType === "date" ? new chartjs.ScatterDateScale(scaleOptions) : new chartjs.ScatterNumberScale(scaleOptions);
		},
